...
运行命令时所在的目录将作为默认 workspace 根目录。`web`、`headless` 和 `desktop` profile 在首次使用时会从随附模板自动初始化；`desktop` 模板由 [Tauri 桌面应用](../desktop/README.zh.md)持有，并要求其启动 token。其他任何 profile 都必须通过 `dsh plugin` 创建。
