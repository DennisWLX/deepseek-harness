...
`web`、`headless` 和 `desktop` profile 首次使用时会从随附模板自动初始化（`web`：base + web-app；`headless`：base + headless；`desktop`：base + web-app + desktop-app）。desktop 模板由 sidecar 持有，不是普通的 `dsh` 命令入口。其他缺失的 profile 会显式报错，并提示运行 `dsh plugin --profile <name> add <package>`。
