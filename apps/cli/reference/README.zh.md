...
`desktop`、`web`、`headless`、`sdk`、`sdk-minimal` 和 `acp` profile 首次使用时会从随附模板自动初始化（`desktop`：base + web-app + desktop-app，实时应用 patch；`web`：base + web-app，实时应用 patch；`headless`：base + headless，只在启动时应用 patch；`sdk`：base + sdk-app，只在启动时应用 patch；`sdk-minimal`：独立组合包，只在启动时应用 patch；`acp`：base + acp-app，只在启动时应用 patch）。desktop 模板由 sidecar 持有，不是普通的 `dsh` 命令入口。其他缺失的 profile 会显式报错，并提示运行 `dsh plugin --profile <name> add <package>`。
