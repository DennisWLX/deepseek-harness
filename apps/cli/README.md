...
The invoking directory is the default workspace root. The `desktop`, `web`, `headless`, `sdk`, `sdk-minimal`, and `acp` profiles auto-initialize on first use from shipped templates; the `desktop` template is owned by the [Tauri desktop application](../desktop/README.md) and requires its launch token. Any other profile must be created through `dsh plugin`.
