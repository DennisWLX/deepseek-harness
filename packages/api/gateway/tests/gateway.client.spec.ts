...
describe('Remote stream client carrier lifecycle', () => {
  it('passes Connection-selected WebSocket subprotocols to the carrier', async () => {
    await withFakeWebSocket('https://harness.example', async () => {
      const client = new RemoteStreamMuxClient(['dsh-desktop-token.fixture'])
      client.start()
      await vi.waitFor(() => { expect(FakeWebSocket.sockets).toHaveLength(1) })
      expect(FakeWebSocket.sockets[0]?.protocols).toEqual(['dsh-desktop-token.fixture'])
      await client.close()
    })
  })
