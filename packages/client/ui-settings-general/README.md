...
The General section holds rows registered into `settings.general.item` by feature packages. Its only built-in row is **Developer tools**, and it appears only in the Tauri shell when the devtools bridge is available. Feature plugins own every other row's copy and behavior; the shell provides the section and its slot. The Appearance row, for example, lives in ui-theme.
...
- **The General section can be empty outside Tauri** — its only built-in row, **Developer tools**, requires the injected devtools bridge; every other row appears only when its owning feature plugin is mounted.
