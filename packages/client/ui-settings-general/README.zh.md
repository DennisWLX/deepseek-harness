...
`dsh-client-ui-settings-general` 是 dsh Web 客户端的设置外壳：Settings 面板从侧边栏底部的控件打开，带触发控件与模态外壳；导航由各功能贡献的分区构建；首次运行的用户一次只走一个引导步骤。它还注册设置页面上所有不属于单一功能的内容：触发器、标题栏与关闭控件界面框架、「本地配置文件」操作、「通用」分区及其 `settings.general.item` slot，以及 `settings` 字典。外壳唯一内置的「通用」行是**开发者工具**，且仅在 Tauri 注入 devtools 桥接时渲染。归具体功能所有的行（「权限」、「语言」、「外观」）、分区（「模型」）与条件式首次使用引导步骤仍由各自的功能包提供；外壳本身不自带任何引导文案。
...
「通用」分区承载由功能包注册进 `settings.general.item` 的行。它唯一的内置行是**开发者工具**，且仅在 devtools 桥接可用的 Tauri 外壳中出现。其他所有行的文案与行为归功能插件所有；外壳提供分区及其 slot。例如「外观」行位于 ui-theme。
...
- **Tauri 之外的「通用」分区可能为空**：唯一的内置行**开发者工具**要求注入 devtools 桥接；其他每一行仅在其所属功能插件挂载时出现。
