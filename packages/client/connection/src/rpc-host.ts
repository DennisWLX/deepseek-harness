...
export class HostConnectionService extends Service implements HostConnectionHandle {
   ...
   * @param accessToken - optional desktop launch token replacing browser-session authentication.
   */
  constructor(
    ctx: Context,
  ...
  requestRejection(request: ConnectionTrustRequest): ConnectionRequestRejection {
    ...
    if (this.accessToken !== undefined) {
      return isDesktopHttpAuthorized(request.headers, this.accessToken) ? undefined : 403
    }
    return this.browserAuth.isAuthenticated(request) ? undefined : 401
  }

  /** Apply the configured Host/Origin fence, then WebSocket authentication. */
  requestUpgradeRejection(request: ConnectionTrustRequest): ConnectionRequestRejection {
    if (!isTrustedApiRequest(request, this.trustedHosts)) return 403
    if (this.accessToken !== undefined) {
      return isDesktopWebSocketAuthorized(request.headers, this.accessToken) ? undefined : 403
    }
    return this.browserAuth.isAuthenticated(request) ? undefined : 401
  }
  ...
  authenticatedUrl(baseUrl: string): string {
    if (this.accessToken !== undefined) return baseUrl
