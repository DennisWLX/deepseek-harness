...
export interface ConnectionHandle {
  ...
  /** WebSocket subprotocols required by the selected physical transport. */
  readonly webSocketProtocols?: readonly string[]
  /**
...
export function apply(ctx: Context): void {
  ...
  const rpc = fixtureRpc ?? createWebConnectionRpc(transport?.fetch, transport?.openStream, accessToken)
  ...
  const handle: ConnectionHandle = {
    ...
    rpc,
    webSocketProtocols: accessToken === undefined ? [] : [desktopWebSocketProtocol(accessToken)],
