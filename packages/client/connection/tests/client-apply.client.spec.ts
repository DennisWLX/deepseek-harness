...
type Win = {
  ...
  __DSH_DESKTOP_ACCESS_TOKEN__?: string
}

afterEach(() => {
  delete (globalThis as Win).location
  delete (globalThis as Win).__DSH_TRANSPORT__
  delete (globalThis as Win).__DSH_DESKTOP_ACCESS_TOKEN__
})
...
describe('connection client apply', () => {
  ...
  it('attaches the desktop launch token to HTTP and WebSocket transports', async () => {
    const token = 'T'.repeat(43)
    ;(globalThis as Win).__DSH_DESKTOP_ACCESS_TOKEN__ = token
    ...
    try {
      const handle = await mount()
      await handle.rpc.call('/api', 'settings/describe', { args: {} })
      const fetch = vi.mocked(globalThis.fetch)
      const headers = fetch.mock.calls[0]?.[1]?.headers
      expect(headers).toMatchObject({ authorization: desktopAuthorizationValue(token) })
      expect(handle.webSocketProtocols).toEqual([desktopWebSocketProtocol(token)])
    } finally {
